...
p = {"status": status, "completed": int(completed), "current": None if current == "none" else current, "total": int(total)}
tmp = path + ".tmp"
...
for unit in "${units[@]}"; do
  ...
  PYTHONPATH=src PYTHONUNBUFFERED=1 "$python_bin" scripts/210_joint_ttt_train.py train \
    --fold "$fold" --seed "$seed" --updates "${JOINT_TTT_UPDATES:-5000}" \
    --batch-size "${JOINT_TTT_BATCH_SIZE:-4}" --micro-contexts "${JOINT_TTT_MICRO_CONTEXTS:-1}" \
    --validation-every "${JOINT_TTT_VALIDATION_EVERY:-100}" --validation-max-rows "${JOINT_TTT_VALIDATION_MAX_ROWS:-64}" \
    --reconstruction-weight "${JOINT_TTT_RECON_WEIGHT:-0.05}" \
    --backbone-lr "${JOINT_TTT_BACKBONE_LR:-1e-5}" --head-lr "${JOINT_TTT_HEAD_LR:-1e-4}" \
    --device "${JOINT_TTT_DEVICE:-cuda}" 2>&1 | tee "$log"
