...
db = lmdb.open(args.processed_dir, map_size=15614542346)

failed_files = []

for file in files:
    try:
        raw = mne.io.read_raw_cnt(os.path.join(root_dir, file), preload=True, data_format="int32")
    ...
    try:
        ...
        print(f"Failed to process {file} during data handling: {e}")
        failed_files.append(file)
        continue
