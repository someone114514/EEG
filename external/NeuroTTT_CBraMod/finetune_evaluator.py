...
class Evaluator:
    ...
    def get_metrics_for_multiclass(self, model):
        ...
        for x, y in tqdm(self.data_loader, mininterval=1):
            ...
            truths += y.cpu().squeeze().detach().numpy().tolist()
            preds += pred_y.cpu().squeeze().detach().numpy().tolist()
    ...
    def get_metrics_for_binaryclass(self, model):
        ...
        for x, y in tqdm(self.data_loader, mininterval=1):
            ...
            truths += y.long().cpu().squeeze().detach().numpy().tolist()
            preds += pred_y.cpu().squeeze().detach().numpy().tolist()
            scores += score_y.cpu().detach().numpy().tolist()
    ...
    def get_metrics_for_regression(self, model):
        ...
        for x, y in tqdm(self.data_loader, mininterval=1):
            ...
            pred = model(x)
            truths += y.cpu().squeeze().detach().numpy().tolist()
            preds += pred.cpu().squeeze().detach().numpy().tolist()
